...
smd(
    ...
    async ({ reply, chat, isGroup, sender, error }) => {
      try {
        ...
        const daily = await eco.daily(sender, "TOFAN BRAND", 500); // Giving 500 coins daily
        if (daily.cd) {
          return await reply(
  ...
  smd(
    ...
    async (message) => {
      try {
        ...
        const balance = await eco.balance(users, "TOFAN BRAND");
        await eco.deduct(users, "TOFAN BRAND", balance.wallet);
...
smd(
    ...
    async (message, match) => {
      try {
        ...
        const balance = await eco.balance(user, "TOFAN BRAND");
        ...
        switch (value) {
          ...
          case "1":
            ...
            await eco.deduct(user, "TOFAN BRAND", 100);
            await eco.giveCapacity(user, "TOFAN BRAND", 1000);
            return await message.reply(
          ...
          case "2":
            ...
            await eco.deduct(user, "TOFAN BRAND", 1000);
            await eco.giveCapacity(user, "TOFAN BRAND", 100000);
            return await message.reply(
          ...
          case "3":
            ...
            await eco.deduct(user, "TOFAN BRAND", 10000);
            await eco.giveCapacity(user, "TOFAN BRAND", 10000000);
            return await message.reply(
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        const deposit = await eco.deposit(message.sender, "TOFAN BRAND", d);
        if (deposit.noten)
...
smd(
    ...
    async (message) => {
      try {
        let h = await eco.lb("TOFAN BRAND", 10);
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        const balance = await eco.balance(user1, "TOFAN BRAND");
        if (balance.wallet < amount)
          return message.reply("💀 You're too broke for this transaction. Try again when you're rich.");
  
        await eco.deduct(user1, "TOFAN BRAND", amount);
        await eco.give(user2, "TOFAN BRAND", amount);
...
smd(
    ...
    async (message) => {
      try {
        ...
        const balance = await eco.balance(message.sender, "TOFAN BRAND");
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        await eco.give(users, "TOFAN BRAND", amount);
  ...
  smd(
    ...
    async (message) => {
      try {
        ...
        const user2 = users;
        const k = 1000;
        const balance1 = await eco.balance(user1, "TOFAN BRAND");
        const balance2 = await eco.balance(user2, "TOFAN BRAND");
        ...
        switch (random) {
          ...
          case "rob":
            ...
            await eco.deduct(user2, "TOFAN BRAND", loot);
            await eco.give(user1, "TOFAN BRAND", loot);
            await message.reply(
          ...
          case "caught":
            ...
            await eco.deduct(user1, "TOFAN BRAND", fine);
            await message.reply(
    ...
    async (message, match) => {
      try {
        ...
        if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0) {
          ...
          const k = 100;
          const balance1 = await eco.balance(user, "TOFAN BRAND");
          if (k > balance1.wallet)
            return message.reply(
          ...
          if (match.split(" ")[0]) {
            ...
            if (value <= balance.wallet) {
              ...
              if (f1 !== f2 && f2 !== f3) {
                const deduct1 = await eco.deduct(user, "TOFAN BRAND", deduff);
                return message.reply(
                  `${mess1}\n\n*Big Lose -->* _🪙${deduff}_`
                );
              } else if (f1 == f2 && f2 == f3) {
                const give1 = await eco.give(user, "TOFAN BRAND", deduff / 2);
                return message.reply(
                  `${mess2}\n*_Little Jackpot -->* _🪙${deduff / 2}_`
                );
              } else if (f1 == f2 && f2 !== f3) {
                const give2 = await eco.give(user, "TOFAN BRAND", deduff);
                return message.reply(`${mess3}\n*Small Win -->* _🪙${deduff}_`);
              } else if (f1 !== f2 && f1 == f3) {
                const deduct2 = await eco.deduct(user, "TOFAN BRAND", deduff);
                return message.reply(
                  `${mess5}\n\n*Small Lose -->* _🪙${deduff}_`
                );
              } else if (f1 !== f2 && f2 == f3) {
                const give4 = eco.give(user, "TOFAN BRAND", deduff);
                return message.reply(
                  `${mess3}\n\n*Small Win -->* _🪙${deduff}_`
                );
              } else if (f1 == f2 && f2 == f3 && f3 == f4) {
                const give5 = eco.give(user, "TOFAN BRAND", deduff * 20);
                return message.reply(
          ...
          if (f1 !== f2 && f2 !== f3) {
            const deduct1 = await eco.deduct(user, "TOFAN BRAND", 50);
            message.reply(`${mess1}\n\n*Big Lose -->* _🪙50_`);
          } else if (f1 == f2 && f2 == f3) {
            const give1 = await eco.give(user, "TOFAN BRAND", 100);
            message.reply(`${mess2}\n*_Little Jackpot -->* _🪙100_`);
          } else if (f1 == f2 && f2 !== f3) {
            const give2 = await eco.give(user, "TOFAN BRAND", 20);
            message.reply(`${mess3}\n*Small Win -->* _🪙20_`);
          } else if (f1 !== f2 && f1 == f3) {
            const deduct2 = await eco.deduct(user, "TOFAN BRAND", 20);
            message.reply(`${mess5}\n\n*Small Lose -->* _🪙20_`);
          } else if (f1 !== f2 && f2 == f3) {
            const give4 = eco.give(user, "TOFAN BRAND", 20);
            message.reply(`${mess3}\n\n*Small Win -->* _🪙20_`);
          } else if (f1 == f2 && f2 == f3 && f3 == f4) {
            const give5 = eco.give(user, "TOFAN BRAND", 1000);
            message.reply(`${mess4}\n\n_🎊 JackPot --> _🪙1000_`);
          } else {
    ...
    async (message) => {
      try {
        ...
        if (mongoschemas == "false")
          return message.reply("*🚦Economy* is not active in current group.");
        const kg = 100;
        const balance1 = await eco.balance(message.sender, "TOFAN BRAND");
        ...
        if (lol) {
          ...
          const give2 = await eco.give(message.sender, "TOFAN BRAND", deduff * 2);
          ...
          const deduct1 = await eco.deduct(message.sender, "TOFAN BRAND", deduff);
          let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `;
          let str = st
            .replace(/1/g, `🔴`)
            .replace(/2/g, `🔵`)
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        const withdraw = await eco.withdraw(user, "TOFAN BRAND", query);
        if (withdraw.noten)
          return message.reply(
        ...
        const add = eco.give(user, "TOFAN BRAND", query);
        message.reply(
    ...
    async (message, match) => {
      try {
        ...
        if (randomDirection === opp) {
          await eco.give(user, "TOFAN BRAND", twice);
          return message.reply(
            `📈 *Damn! You actually won 🪙${twice}!* But don’t get too happy, it’s just luck. 🤨`
          );
        } else {
          await eco.deduct(user, "TOFAN BRAND", texts[0]);
          return message.reply(
